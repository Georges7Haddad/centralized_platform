...
def read_course(course_crn: int):
	session: Session = Depends(get_session)
...
def register_user_to_course(user_id: int, course_crn: int):
	session: Session = Depends(get_session)
...
def read_user_courses(user_id: int):
	session: Session = Depends(get_session)
...
def read_course_users(course_crn: int):
	session: Session = Depends(get_session)
	course_users = session.exec(
		select(UserCourseLink).where(UserCourseLink.course_crn == course_crn)
	).all()
	if not course_users:
		raise HTTPException(status_code=404, detail="Course not found")
	return course_users
