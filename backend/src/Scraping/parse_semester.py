...
# parses the courses of a semester
def parse_courses(html):
	...
	for th in course_rows:
		...
		term_match = re.search(
			r"Associated Term:\s*(.*?)\s+Registration Dates:", detail_text
		)
		...
		time_str = (
			data_cells[1].get_text(strip=True) if len(data_cells) > 1 else ""
		)
		days_str = (
			data_cells[2].get_text(strip=True) if len(data_cells) > 2 else ""
		)
		where_str = (
			data_cells[3].get_text(strip=True) if len(data_cells) > 3 else ""
		)
		schedule_type_str = (
			data_cells[5].get_text(strip=True) if len(data_cells) > 5 else ""
		)
		instructor_str = (
			data_cells[6].get_text(strip=True) if len(data_cells) > 6 else ""
		)
